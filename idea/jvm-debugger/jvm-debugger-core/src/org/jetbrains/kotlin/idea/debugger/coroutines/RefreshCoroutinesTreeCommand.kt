...
class RefreshCoroutinesTreeCommand(
        ...
        if (context.debuggerSession is DebuggerSession && sc is SuspendContextImpl && !sc.isResumed) {
            ...
            if (infoCache.isOk()) {
                ...
                for (state in infoCache.cache) {
                    val descriptor = createDescriptorNode(nf, state, evaluationContext)
                    root.add(descriptor)
                }
                setRoot(root)
            } else {
    ...
    private fun createDescriptorNode(
        nodeFactory: NodeManagerImpl,
        coroutineState: CoroutineState,
        evaluationContext: EvaluationContextImpl
    ) = nodeFactory.createNode(nodeFactory.getDescriptor(null, CoroutineData(coroutineState)), evaluationContext)
